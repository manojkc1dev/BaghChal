  ...
  if (type === 'LION') {
    return (
      <motion.div
        ...
        animate={{ scale: isSelected ? 1.15 : 1, rotate: 0 }}
        exit={{ scale: 0, opacity: 0 }}
        transition={{ type: 'spring', stiffness: 400, damping: 25 }}
        whileHover={{ scale: isSelected ? 1.2 : 1.08 }}
        whileTap={{ scale: 0.95 }}
      ...
      >
        ...
        <motion.div
          animate={{
            opacity: isSelected ? [0.6, 1, 0.6] : 0.6,
            scale: isSelected ? [1, 1.1, 1] : 1,
          }}
          transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
          className="absolute inset-0 rounded-full bg-gradient-to-r from-red-600 to-amber-600 blur-md pointer-events-none"
        />

        {/* Piece Body */}
        <div
          className={`relative w-11 h-11 sm:w-13 sm:h-13 rounded-full bg-gradient-to-br from-red-950 via-red-900 to-amber-950 border-2 ${
            isSelected
              ? 'border-amber-400 shadow-xl shadow-red-500/60 ring-2 ring-amber-400/50'
              : 'border-red-500/60 shadow-md shadow-red-950/80'
          } flex items-center justify-center text-amber-300 select-none`}
        >
          <svg
            viewBox="0 0 24 24"
            className="w-7 h-7 sm:w-8 sm:h-8 fill-current text-amber-400 drop-shadow-md"
  ...
  if (type === 'SHEEP') {
    return (
      ...
      >
        ...
        <motion.div
          animate={{
            opacity: isSelected ? [0.5, 0.9, 0.5] : 0.5,
            scale: isSelected ? [1, 1.1, 1] : 1,
          }}
          transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
          className="absolute inset-0 rounded-full bg-gradient-to-r from-sky-400 to-emerald-400 blur-md pointer-events-none"
        />

        {/* Piece Body */}
        <div
          className={`relative w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-slate-900 via-slate-800 to-sky-950 border-2 ${
            isSelected
              ? 'border-sky-300 shadow-xl shadow-sky-500/60 ring-2 ring-sky-300/50'
              : 'border-sky-400/60 shadow-md shadow-slate-950/80'
          } flex items-center justify-center text-sky-200 select-none`}
        >
          <svg
            viewBox="0 0 24 24"
            className="w-6 h-6 sm:w-7 sm:h-7 fill-current text-sky-200 drop-shadow-md"
            xmlns="http://www.w3.org/2000/svg"
          >
